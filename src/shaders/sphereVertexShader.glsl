...
float waveFunction(vec3 position, int waveCount, float time) {
    float frequency = float(waveCount);
    return sin(position.y * frequency + time);
}
...
void main() {
    ...
    float displacement = combinedNoise;
